...
public abstract class SduCalciteOptimizer {
  ...
  protected RelNode runVolcanoPlanner(RuleSet ruleSet, RelNode input, RelTraitSet targetTraits) {
     ...
     *
     * 3: RelSubset Importance
     *
     *    假设RelNode关系表达式Cost是3, 对应的Importance是0.5, 这个RelNode有两个输入, 对应的RelSubSet为s1, s2(假设已没有输入), 其Cost分别为2和5
     *
     *    s1的Importance: 2 / (3 + 2 + 5) * 0.5
     *
     *    s2的Importance: 5 / (3 + 2 + 5) * 0.5
     *
     * 3: VolcanoPlanner.setRoot()
     *
     *    3.1 RelNode转为RelSubSet, 同时创建RelNode等价关系表达式集合RelSet
     *
     *    3.2 初始化关系表达式(RelNode)的等价关系集合(RelSet)
     *
     *                 Syntax Tree                    =====>>      Equivalence-Set Of Expressions
     *
     *                 Project(#3)                                          RelSubSet[RelSet(Project(#3))]
     *                    /|\                                                            /|\
     *                     |                                                              |
     *          +----------+---------+                                  +-----------------+-----------------+
     *          |                    |                                  |                                   |
     *          |                    |                                  |                                   |
     *      TableScan(#1)      TableScan(#2)                RelSubSet[RelSet(TableScan(#1))]      RelSubSet[RelSet(TableScan(#2))]
     *
     * 4: VolcanoRuleCall.transform()
     *
     * 5: VolcanoPlanner.findBestExp()
     *
     *
     * */
